...
export const getCars = () => {
  return async (dispatch) => {
    ...
    let cars = await axios.get('http://localhost:3000/')
    dispatch({
...
export const createCar = (newCar) => {
  return async (dispatch) => {
    ...
    let newcar = await axios.post('http://localhost:3000/', newCar)
    dispatch({
...
export const editCar = (editedCar) => {
  ...
  return async (dispatch) => {
    ...
    let updatedcar = await axios.patch(`http://localhost:3000/${editedCar.id}`, bodyObj)
    dispatch({
...
export const deleteCar = (carId) => {
  return async (dispatch) => {
    ...
    let removedcar = await axios.delete(`http://localhost:3000/${carId}`)
    dispatch({
